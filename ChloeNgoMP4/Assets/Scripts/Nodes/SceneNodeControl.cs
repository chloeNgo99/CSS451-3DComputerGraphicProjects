...
public class SceneNodeControl : MonoBehaviour {
    ...
    void Start () {
        ...
        mSelectMenuOptions.Add(new Dropdown.OptionData(BaseNode.transform.name));
        mSelectedTransform.Add(BaseNode.transform);
        GetChildrenNames("", BaseNode.transform);
        TheMenu.AddOptions(mSelectMenuOptions);
        TheMenu.onValueChanged.AddListener(SelectionChange);

        XformControl.SetSelectedObject(BaseNode.transform);
    }
